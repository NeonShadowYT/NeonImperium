...
(function() {
  ...
  async function fetchAllReleasesWithRetry(signal) {
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        const resp = await fetch(url, { headers, signal: signal || currentAbortController.signal });
        ...
        console.warn(`[platform] Releases fetch attempt ${attempt+1} failed:`, err);
  ...
  async function getGameReleases(gameTag) {
    const all = await fetchAllReleasesWithRetry();
  ...
  window.initPlatform = async function() {
    ...
    try {
      ...
      console.error('[platform] Failed to load releases:', e);
