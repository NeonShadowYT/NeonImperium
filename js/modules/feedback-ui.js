// feedback-ui.js – рендер постов, реакции, комментарии, опросы
(function() {
    ...
    // ---------- Вспомогательные функции для комментариев и ссылок на посты ----------
    ...
    async function renderComments(container, comments, currentUser, issueNumber) {
        ...
        for (const c of regularComments) {
            ...
            commentDiv.innerHTML = `<div class="comment-meta"><span class="comment-author">${GithubCore.escapeHtml(c.user.login)}</span></div><div class="comment-body">${GithubCore.escapeHtml(c.body).replace(/\n/g,'<br>')}</div><div class="comment-mini-cards"></div>${actionsHtml}`;
    ...
    // ---------- Рендер тела поста (Markdown + встроенные ссылки) ----------
    ...
    function setupCommentForm(container, item, currentUser) {
        ...
        commentForm.innerHTML = `<div style="display: flex; gap: 8px; width: 100%; align-items: stretch;"><input type="text" class="comment-input" placeholder="Написать комментарий..." style="flex: 1; height: 40px;"><button class="button comment-submit" style="flex-shrink: 0; height: 40px;">Отправить</button><button class="button comment-editor-btn" style="flex-shrink: 0; height: 40px; padding: 0 12px;" title="Редактор"><i class="fas fa-pencil-alt"></i></button></div>`;
    ...
    // ---------- Экспорт для совместимости с оригинальным кодом ----------
