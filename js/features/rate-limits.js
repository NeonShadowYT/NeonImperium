...
(function() {
  ...
  const HISTORY_KEY = 'rate_history';
  ...
  function addHistory(action, data, status) {
    let history = [];
    try {
      const encrypted = localStorage.getItem(HISTORY_KEY);
      if (encrypted) {
        const decrypted = xorDecrypt(encrypted, ENCRYPTION_SALT);
    ...
    const encrypted = xorEncrypt(JSON.stringify(history), ENCRYPTION_SALT);
  ...
  async function clearAllCacheInternal() {
    ...
    const exclude = ['rate_limits', 'rate_history', 'license_agreed_v1', 'license_version', 'license_agreed_timestamp', 'preferredLanguage', 'github_token', 'github_token_local', 'remember_me', 'last_cache_clear'];
    // Не удаляем зашифрованную историю и лимиты
  ...
  function openRatePanel() {
    ...
    loadQueueItems(modal, t);
    loadHistoryItems(modal, t);
    loadCacheItems(modal, t); // исправлено: передаём t
  }
  ...
  function bindPanelEvents(modal, t) {
    modal.querySelectorAll('.rate-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        if (target === 'cache') loadCacheItems(modal, t);
      });
    });
  ...
  function loadCacheItems(modal, t) {
