...
// Реэкспорт утилит. cacheGet/cacheSet теперь асинхронные (шифрование через CacheCrypto).
// Конфигурация берётся из window.NeonConfig (js/config.js).
(function() {
    // ---- Централизованная конфигурация с fallback на дефолты ----
    const NeonConfig = (typeof window !== 'undefined' && window.NeonConfig) || {};
    const CONFIG = {
        REPO_OWNER: NeonConfig.REPO_OWNER || 'NeonShadowYT',
        REPO_NAME: NeonConfig.REPO_NAME || 'NeonImperium',
        CACHE_TTL: NeonConfig.CACHE_TTL || 10 * 60 * 1000,
        API_CACHE_TTL: NeonConfig.API_CACHE_TTL || 5 * 60 * 1000,
        IMAGE_CACHE_TTL: NeonConfig.IMAGE_CACHE_TTL || 30 * 24 * 60 * 60 * 1000,
        RELEASES_CACHE_TTL: NeonConfig.RELEASES_CACHE_TTL || 60 * 60 * 1000,
        ALLOWED_AUTHORS: NeonConfig.ALLOWED_AUTHORS || ['NeonShadowYT', 'GoldenCreeper567']
    };
